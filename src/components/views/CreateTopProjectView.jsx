...
import { FolderOpen, Calendar, Tag, ChevronLeft, ChevronRight } from 'lucide-react';
import ProjectPartnerMappingStep from './ProjectPartnerMappingStep';
...
const CreateTopProjectView = ({ onBack, isModal = false }) => {
  ...
  const handleSubmit = () => {
    const projectData = {
      ...
      partnerMappings: selectedPartners
    };
  ...
  const card = (
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{
              ...
              height: 32,
              borderRadius: '50%',
              background: currentStep >= 1 ? 'var(--color-primary)' : '#e5e7eb',
              color: 'white',
              display: 'flex',
              alignItems: 'center',
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: currentStep >= 1 ? '#111827' : '#6b7280' }}>
              Project Details
            </span>
          </div>
          <div style={{
            width: 32,
            height: 2,
            background: currentStep === 2 ? 'var(--color-primary)' : '#e5e7eb',
            flex: 1,
            margin: '0 12px'
          }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: currentStep === 2 ? '#111827' : '#6b7280' }}>
              Partner Mapping
            </span>
          </div>
        </div>
      ...
      {/* Step Actions */}
      <div style={{ display:'flex', justifyContent:'space-between', gap:10, padding:'16px 24px', borderTop:'1px solid #e5e7eb', background:'#fafafa' }}>
        <button onClick={handleBack} style={{ padding:'10px 16px', border:'1px solid #e5e7eb', background:'white', color:'#374151', borderRadius:8, cursor:'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, fontSize: 13 }}>
          <ChevronLeft size={16} /> Back
        </button>
        {currentStep === 1 ? (
          <button onClick={handleNext} style={{ padding:'10px 16px', background:'linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-2) 100%)', color:'white', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700, display: 'flex', alignItems: 'center', gap: 6 }}>
            Next <ChevronRight size={16} />
          </button>
        ) : (
          <button onClick={handleSubmit} style={{ padding:'10px 16px', background:'linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-2) 100%)', color:'white', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700 }}>
            Create Project
          </button>
        )}
      </div>
  ...
  const fieldsOnly = (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: currentStep >= 1 ? '#111827' : '#6b7280', flex: 1 }}>Project Details</span>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: currentStep === 2 ? '#111827' : '#6b7280', flex: 1 }}>Partner Mapping</span>
      </div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', gap:10, paddingTop:16 }}>
        <button onClick={handleBack} style={{ padding:'10px 16px', border:'1px solid #e5e7eb', background:'white', color:'#374151', borderRadius:8, cursor:'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, fontSize: 13 }}>
          <ChevronLeft size={16} /> Back
        </button>
        {currentStep === 1 ? (
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: 16 }}>
        ...
        <button onClick={handleBack} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 12px', border:'1px solid #e5e7eb', background:'white', borderRadius:8, cursor:'pointer', color:'#374151' }}>
          <ChevronLeft size={16} /> Back
        </button>
      </div>
